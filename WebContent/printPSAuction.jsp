			...
			for(int i=0; i<auc.size(); i++){
				for(int j=0; j<ps.size(); j++){
					if(auc.get(i).getAuctionId() == ID && ps.get(j).getId() == auc.get(i).getItemId()){
						...
						<% if(auc.get(i).getWinnerEmail() != null){ %>
							<p style='text-align:center; font-family:Sans-serif; font-size:20px;'>Winner: <%= auc.get(i).getWinnerEmail() %></p>
						<% }else {%>
						...
						<form action="enterBid.jsp">
							...
							<input type="hidden" value="<%= auc.get(i).getAutowin() %>" name="aw">							
						...
						<form action="bidHistory.jsp">
							<h3 style="text-align:center; font-family:Sans-serif">Click Below To View The History Of Bids For This Item</h3> 
							<input type="hidden" value="<%= auc.get(i).getAuctionId() %>" name="aid">

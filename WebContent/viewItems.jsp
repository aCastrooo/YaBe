...
<body>
	...
	<%
		...
		LinkedList<Integer> a = dao.getIDFromAuction();
		...
		for(int j = i.size()-1; j >= 0; j--){
			...
			if(seshEmail.equals(i.get(j).getEmail())){
					...
					for(int f=0; f<x.size(); f++){
						...
						if(x.get(f).getId() == i.get(j).getId()){
							...
							for(int y = 0; y<a.size(); y++){
								...
								if(a.get(y) == i.get(j).getId()){
									%>
									<p style='text-align:center; font-family:Sans-serif; font-size:20px;'>On Auction</p>
									<%
					...
					for(int f=0; f<p.size(); f++){
						...
						<br>
						<p style='text-align:center; font-family:Sans-serif; font-size:20px;'>Condition: <%= i.get(j).getCond() %></p>
						<p style='text-align:center; font-family:Sans-serif; font-size:20px;'>Minimum CPU: <%= p.get(f).getCPU() %> MHz</p>
						<p style='text-align:center; font-family:Sans-serif; font-size:20px;'>Minimum RAM: <%= p.get(f).getRAM() %> GB</p>
